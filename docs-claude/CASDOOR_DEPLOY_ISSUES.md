...
> 记录日期：2026-04-26  
> 适用项目：investment-app / info-app（两者共享同一套 Casdoor 部署方案）
...
从 **info-app `init.sh` 派生的新项目**在 k8s 侧落地后：把 `.conf` 里的组织名、应用名、`redirect_uris`、client_id 改成新项目域名；首次 Casdoor 初始化务必执行上述脚本或等价 SQL（见下文「问题 11 / 12」）。
...
**影响组件**：`investment-admin-backend`、`info-admin-backend`
...
| `info-admin-backend/` 对应文件 | 与 `investment-admin-backend/` 同步所有上述修改 |
...
（派生新项目时：**无需**把上述脚本复制进 info-app；在 **k8s 仓库**维护 Casdoor，info-app 侧仅复制业务 submodule 与文档约定。）
