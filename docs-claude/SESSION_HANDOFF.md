...
**最后完成的工作**：
- info-web-backend（NestJS）启动修复：SWC 路径别名永久修复（tsc-alias），不再需要 runtime bootstrap
- info-web-frontend auth 流程打通：登录 → Casdoor OIDC → callback → Redis session → dashboard
...
- info-admin-backend（FastAPI/Python）初步启动：uv 安装 Python，创建 info_admin 数据库，修复 alembic 冲突

---

## 下一步要做什么

**下一个任务**：完成 info-admin 前后端联调，测试 admin 登录流程

按顺序执行：
1. 等 pnpm install 完成后启动 admin 前端：`pnpm dev --host`（端口 5173）
2. 在 Casdoor 管理后台为 `app-info-admin` 应用添加 redirect URI：`http://43.159.148.235:8001/auth/callback`
...
| 服务 | 启动命令 | 端口 |
|------|---------|------|
| info-web-backend | `node dist/main`（在 info-web-backend/app/） | 8000 |
| info-web-frontend | `npm run dev`（在 info-web-frontend/app/） | 3000 |
| info-admin-backend | `uv run uvicorn app.main:app --host 0.0.0.0 --port 8001`（在 info-admin-backend/app/） | 8001 |
| info-admin-frontend | `pnpm dev --host`（在 info-admin-frontend/，需先 pnpm install） | 5173 |
...
| 路径 | 说明 |
|------|------|
| `info-web-backend/app/.env` | 后端环境变量（含 NODE_TLS_REJECT_UNAUTHORIZED=0） |
| `info-web-frontend/app/.env.local` | 前端环境变量（NEXT_PUBLIC_API_URL） |
| `info-admin-backend/app/.env` | admin 后端（CASDOOR_REDIRECT_URI 已改为外网 IP） |
| `info-admin-frontend/.env` | admin 前端（VITE_API_URL 已改为外网 IP） |
...
| info-admin-frontend pnpm install 可能还在进行 | 完成后需启动 |
