...
CONFIG_FILE="$SCRIPT_DIR/deploy-__APP_NAME__-secret.conf"
# 从 deploy-__APP_NAME__-secret/ -> __APP_NAME__-secret/ -> secret/ -> deploy-__APP_NAME__/ -> __APP_NAME__/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
SECRET_YAML="$K8S_RESOURCE_DIR/custom-values/secret/__APP_NAME__-secret/generate-__APP_NAME__-secret/__APP_NAME__-secret-generated.yaml"
...
main() {
    ...
    local generate_script="$K8S_RESOURCE_DIR/custom-values/secret/__APP_NAME__-secret/generate-__APP_NAME__-secret/generate-__APP_NAME__-secret.sh"
    ...
    case "$action" in
        deploy)
            ...
            log_success "__APP_NAME__ Secret 部署完成"
            ;;
        uninstall)
            kubectl delete -f "$SECRET_YAML" -n "$NAMESPACE" --ignore-not-found
            log_success "__APP_NAME__ Secret 卸载完成"
            ;;
        status)
            kubectl get secret "__APP_NAME__-secret" -n "$NAMESPACE" 2>/dev/null \
                || log_warn "Secret 不存在: __APP_NAME__-secret"
            ;;
        generate)
