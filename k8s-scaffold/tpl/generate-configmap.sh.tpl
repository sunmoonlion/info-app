#!/bin/bash
# __APP_NAME__ ConfigMap YAML 生成脚本

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-__APP_NAME__-config.conf"
# 从 generate-__APP_NAME__-config/ -> __APP_NAME__-config/ -> configMap/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 从 generate-__APP_NAME__-config/ -> ... -> k8s-resource/ -> resources/ -> __APP_NAME__/
...
main() {
    log_info "开始生成 __APP_NAME__ ConfigMap YAML 文件..."
