...
├── info-admin-frontend/      # 管理后台前端（Vue 3 + Vite，CSR）
├── info-admin-backend/       # 管理后台后端（FastAPI + SQLAlchemy）
├── info-web-frontend/        # 用户端前端（Next.js 16，SSR）
└── info-web-backend/         # 用户端 BFF 后端（NestJS + TypeScript）
```
...
| 子模块 | 技术栈 | 说明 |
|--------|--------|------|
| info-admin-frontend | Vue 3 + Vite | 管理后台，CSR 模式 |
| info-admin-backend | FastAPI + Python 3.12 | 管理后台后端，DDD 架构，Casdoor BFF 认证 |
| info-web-frontend | Next.js 16 + shadcn/ui | 用户端前端，SSR 模式，Casdoor BFF 认证 |
| info-web-backend | NestJS + TypeScript | 用户端 BFF 后端，Casdoor OIDC 对接 |
...
```
管理端：info-admin-frontend  +  info-admin-backend
用户端：info-web-frontend    +  info-web-backend
```

BFF 认证说明：
- 当前建议采用「后端 BFF 统一对接 Casdoor」模式：
  - `info-web-frontend` 跳转到 `info-web-backend /auth/login`
  - `info-admin-frontend` 跳转到 `info-admin-backend /auth/login`
...
2. 回到**父仓库根目录**（如 `info-app`）：`git add` → `git commit` → `git push`，把子模块目录在父仓库里记录的**提交指针**一并推上去。
...
| 场景 | 常用命令 |
|------|----------|
| 只把「某子模块」更新后的指针写进父仓库 | `git add info-admin-frontend`（目录名即 `.gitmodules` 里的 `path`） |
...
注意：子模块**内部的**源码改动，只能在**进入该子模块目录后**用 `git add` / `commit`，不要在父仓库里对 `info-xxx/src/...` 单独 `add`。
...
**命令示例**（假设父仓库根目录名为 `info-app`）：

```bash
# 例 1：只更新「管理端前端」子模块指针（子模块里已 commit + push 完毕）
cd info-app
git add info-admin-frontend
git commit -m "chore: bump info-admin-frontend"
git push origin master
...
```bash
git clone --recurse-submodules https://gitee.com/sunmoonlion/info-app.git
```

**后续更新**（本地已有父仓库，只同步远程最新代码时）：

```bash
cd <父仓库根目录>   # 例如 info-app
git pull
...
当 `info-app` 中任一 backend（如 `info-admin-backend` / `info-web-backend`）需要数据库时，使用：

- 业务侧脚手架：
  - `info-admin-backend/db-access-bootstrap/`
  - `info-web-backend/db-access-bootstrap/`
- 底层开通能力（**各 backend 目录内各有一份，互不同步；与 `k8s` 仓库解耦**）：
  - `info-admin-backend/db-provisioner/bin/dbctl`
  - `info-web-backend/db-provisioner/bin/dbctl`
...
```bash
cd info-admin-backend/db-access-bootstrap   # 或 info-web-backend/db-access-bootstrap
...
```bash
git clone --recurse-submodules https://gitee.com/sunmoonlion/info-app.git <新项目名>-app
cd <新项目名>-app
```
...
脚本会自动完成：
- 将四个子模块内所有文件中的 `info` / `Info` / `INFO` 替换为项目名
- 重命名四个子模块目录，并修正各子模块 `.git` 指针
- 更新 `.gitmodules` 中的远程 URL
- 若当前是 Git 克隆：同步父仓 `.git/modules/*`、各子模块的 `worktree` 与远程 URL、父仓 `.git/config` 中的子模块段，并把索引里的子模块路径从 `info-*` 改为 `<项目名>-*`（避免仅改目录名后 `git submodule` 断裂）
