...
```bash
# 1. 进入子模块
cd info-admin-frontend   # 或其他三个模块
...
| 子模块 | 本地镜像名 | Harbor 全名 |
|---|---|---|
| info-admin-frontend | `info-admin-frontend:1.0.0` | `harbor.sunmoonai.com:30443/k8s-images/info-admin-frontend:<tag>` |
| info-admin-backend | `info-admin-backend:1.0.0` | `harbor.sunmoonai.com:30443/k8s-images/info-admin-backend:<tag>` |
| info-web-frontend | `info-web-frontend:1.0.0` | `harbor.sunmoonai.com:30443/k8s-images/info-web-frontend:<tag>` |
| info-web-backend | `info-web-backend:1.0.0` | `harbor.sunmoonai.com:30443/k8s-images/info-web-backend:<tag>` |

**Tag 规则**：`<git-sha>`（不可变，生产使用）+ `latest`（移动引用，仅开发联调）。  
**实例化**：`info-` 前缀由 `init.sh` 替换为业务名，同步修改各模块 `mybuild/build.conf` 中的 `IMAGE` 变量。
...
| 模块 | 镜像名变量 | Tag 变量 |
|---|---|---|
| info-admin-frontend | `ADMIN_CSR_IMAGE` | `ADMIN_CSR_TAG` |
| info-admin-backend | `ADMIN_BACKEND_IMAGE` | `ADMIN_BACKEND_TAG` |
| info-web-frontend | `INFO_SSR_IMAGE` | `INFO_SSR_TAG` |
| info-web-backend | `WEB_BACKEND_IMAGE` | `WEB_BACKEND_TAG` |
...
## 一、info-admin-frontend（Vue 3 + Vite → nginx）

### 手动构建

```bash
cd info-admin-frontend
...
## 二、info-admin-backend（FastAPI + Python 3.12）

### 手动构建

```bash
cd info-admin-backend

# 黄金命令
docker build -f mybuild/Dockerfile \
  --build-arg REGISTRY=harbor.sunmoonai.com:30443/k8s-images \
  -t info-admin-backend:1.0.0 .
...
--context       <子模块根目录>
--build-arg     REGISTRY=harbor.sunmoonai.com:30443/k8s-images
--destination   harbor.sunmoonai.com:30443/k8s-images/info-admin-backend:<git-sha>
--destination   harbor.sunmoonai.com:30443/k8s-images/info-admin-backend:latest
--insecure
--skip-tls-verify
```
...
## 三、info-web-frontend（Next.js 16 SSR）

### 手动构建

```bash
cd info-web-frontend

# 黄金命令（--target 可省略，run-minimal 是最后一个 stage，默认构建）
docker build -f mybuild/Dockerfile \
  --build-arg REGISTRY=harbor.sunmoonai.com:30443/k8s-images \
  --build-arg NEXT_PUBLIC_API_URL=http://localhost:8000 \
  -t info-web-frontend:1.0.0 .
...
| `NEXT_PUBLIC_APP_NAME` | 应用名称 | `info` |
...
--destination   harbor.sunmoonai.com:30443/k8s-images/info-web-frontend:<git-sha>
--destination   harbor.sunmoonai.com:30443/k8s-images/info-web-frontend:latest
--insecure
--skip-tls-verify
```
...
## 四、info-web-backend（NestJS + TypeScript）
...
--target        run
--build-arg     REGISTRY=harbor.sunmoonai.com:30443/k8s-images
--destination   harbor.sunmoonai.com:30443/k8s-images/info-web-backend:<git-sha>
--destination   harbor.sunmoonai.com:30443/k8s-images/info-web-backend:latest
--insecure
--skip-tls-verify
```
...
- **非 root**：info-web-backend 运行时使用 `appuser`，其余模块按需跟进
...
□ 3. info-admin-frontend：确认 VITE_API_URL 值，修改 build.conf
□ 4. info-admin-backend：确认 app/uv.lock 已提交
□ 5. info-web-frontend：确认 NEXT_PUBLIC_API_URL 值，修改 build.conf
□ 6. info-web-backend：确认 CI 只引用主 Dockerfile
...
└── k8s-images/                         ← Harbor 项目（需提前在 Harbor UI 创建）
    ...
    ├── python:3.12-slim
    ├── nginx:stable-alpine
    ├── info-admin-frontend:<git-sha>    ← 应用镜像
    ├── info-admin-backend:<git-sha>
    ├── info-web-frontend:<git-sha>
    └── info-web-backend:<git-sha>
```
