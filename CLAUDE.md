...
2. 核对目标子仓代码、README、OpenAPI/migration、测试和当前 Git 状态。
3. 涉及 MoocManus、Artifact、Identity 或前端迁移时，以 k8s 仓库的 v5 总体方案、实施计划、ADR、contracts 和 evidence 为准。

## 工作边界

- Info 拥有来源、文档版本、Artifact 和 Delivery 领域事实，不拥有 Retrieval、Agent Runtime 或长期记忆。
- 接口事实来自代码/OpenAPI/schema 和 contract tests；`docs/history/` 只用于审计。
- 任何时刻只推进实施计划中一个已激活任务；不得用 mock success 替代真实跨仓验证。
