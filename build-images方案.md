...
| 子模块 | 状态 | Dockerfile 位置 | 关键禁忌 |
|--------|------|-----------------|---------|
| tpl-admin-frontend | ❌ 阻断 | 根目录（3 个文件）| `FROM dev:1.0` CI 无此镜像；`Dockerfile` 要求预置 `dist/` |
| tpl-admin-backend | ⚠️ 路径不规范 | `app/Dockerfile` | Kaniko 须显式 `--dockerfile`，否则找不到 |
| tpl-web-frontend | ✅ 小改即可 | `mybuild/Dockerfile` | `build.conf` 中 `tpl-app-ssr` 与目录名不一致，实例化时修正 |
| tpl-web-backend | ✅ 主文件可用 | `mybuild/Dockerfile` | **禁止** CI 指向 `Dockerfile.original`（含 `--mount=type=cache`，Kaniko 失败）|
...
> **Harbor 项目名决策**：`k8s-images` 与 `apps` 二选一，**全局统一**，不可混用。本文默认 `k8s-images`；若改用 `apps`，全文替换。
...
各子模块 `mybuild/build.conf`（或根目录 `build.conf`）须包含以下最小字段，TAG 不写死，由 CI 注入：

```bash
IMAGE_NAME=tpl-admin-frontend   # 替换为实际服务名
REGISTRY=harbor.sunmoonai.com:30443
PROJECT=k8s-images
# TAG 由 CI 传入，如: IMAGE_TAG=${GIT_SHA}
```
...
```
□ 0. 确认 Harbor 项目名（k8s-images 或 apps），全局统一后再执行后续步骤（见 §五）
...
□ 5. tpl-web-backend：改造 mybuild/Dockerfile 替换镜像源；确认 CI 只引用主 Dockerfile，禁止指向 Dockerfile.original（见 §3.4）
□ 6. 各子模块补充/更新 build.conf 最小字段（见 §五）
□ 7. 各子模块 README 写清一条等价 docker build 命令（黄金命令，供本地/CI 对照）
□ 8. 本地验证：docker build 各子模块，确认容器可正常启动
□ 9. 推送测试镜像到 Harbor，验证 <name>:<git-sha> 格式正确
□ 10. 接入 Argo Workflows WorkflowTemplate，按 §六 配置各子模块参数
□ 11. 过渡期：Jenkins Pipeline 按相同命名/tag 规则执行，与新方案语义对齐
```
